...
module.exports = {
    ...
    selfies : async (req, res) => {
        const userIdx = req.decoded.userIdx;
        const selfies = req.files;
        const result = await UserModel.getSelfies(userIdx);
        const path = selfies.map(img => img.location);
        var tmp = result[0].selfies;
        //console.log(tmp);
        if(tmp !== null)
        {
            tmp = tmp +','+ path.join(',');
        ...
        const result2 = await UserModel.updateSelfies(userIdx,tmp);
        if(selfies === undefined)
        {
            res.status(CODE.OK).send(util.fail(CODE.OK,MSG.NULL_VALUE));
        }
        res.status(CODE.OK).send(util.success(CODE.OK,MSG.SELFIES_SUCCESS,tmp));
    }
}