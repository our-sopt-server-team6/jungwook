...
const user = {
    ...
    updateSelfies : async (userIdx, selfies) => {
        const query = `UPDATE ${table} SET selfies="${selfies}" WHERE userIdx="${userIdx}"`;
        try{
            const result = await pool.queryParam(query);
            return result;
        }catch(err){
    ...
    getSelfies : async (userIdx, selfies) => {
        const query = `SELECT selfies FROM ${table} WHERE userIDx="${userIdx}"`;
        try{
            const result = await pool.queryParam(query);
            return result;
        }catch(err)
        {
            console.log('getSelfies ERROR :',err);
            throw err;
        }
    }
}
